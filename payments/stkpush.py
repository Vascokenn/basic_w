...
def initiate_stk_push(request):
    ...
    if isinstance(access_token_response, JsonResponse):
        ...
        if access_token:
            amount = 1
            #phone = "254723754676"
            #obtain phone number from form
            #phone = request.POST.get('mpesa_number', '')
            raw_phone = request.POST.get('mpesa_number', '')
            # Strip the leading '0' and replace it with '254'
            phone = '254' + raw_phone[1:] if raw_phone.startswith('0') else raw_phone
            passkey = "bfb279f9aa9bdbcf158e97dd71a467cd2e0c893059b10f78e6b72ada1ed2c919"
            business_short_code = '174379'
            process_request_url = 'https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest'
            callback_url = 'https://66bb-102-211-145-33.ngrok-free.app/'
            timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
            password = base64.b64encode((business_short_code + passkey + timestamp).encode()).decode()
            party_a = phone
            party_b = '254708374149'
            account_reference = 'VASC0TECH SOLUTIONS'
